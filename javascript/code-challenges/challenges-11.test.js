...
const divisibleByFiveTwoToThePower = (input) => {
  ...
  let newArray = [];
  input.map(arr => {
    newArray.push(arr.filter(n => typeof n === 'number' && n % 5 === 0).map(n => Math.pow(2, n)))
  });
  return newArray;
};
...
let findMaleAndFemale = (data) => {
  // Solution code here...
  return data.filter((char) => char.gender === 'male' || char.gender === 'female').map(item => item['name']).join(' and ');
